...
function renderLuxeThumbs() {
    ...
    luxeGalleryItems.forEach((src, index) => {
        const button = document.createElement('button');
        const image = document.createElement('img');
        const isActive = index === luxeGalleryIndex;

        button.type = 'button';
        button.className = `luxe-gallery__thumb${isActive ? ' is-active' : ''}`;
        button.dataset.index = String(index);
        button.setAttribute('aria-label', `Изображение ${index + 1}`);
        button.setAttribute('aria-current', isActive ? 'true' : 'false');

        image.src = src;
        image.alt = `Миниатюра проекта ARCHISPACE ${index + 1}`;
        image.loading = 'lazy';
    ...
    luxeGalleryThumbs.replaceChildren(fragment);
}
