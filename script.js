...
function insertReviewToTop(review) {
    reviewsCache = [review, ...reviewsCache];
    renderReviewsList();
...
function updateLoadMoreButton() {
    ...
    loadMoreReviewsBtn.disabled = isLoadingMoreReviews;
    loadMoreReviewsBtn.textContent = isLoadingMoreReviews ? 'Загрузка...' : 'Показать ещё';
}
...
async function loadInitialReviews() {
    try {
        ...
        hasMoreReviews = snapshot.docs.length === REVIEWS_PAGE_SIZE;
...
async function loadMoreReviews() {
    if (!hasMoreReviews || !lastVisibleReviewDoc || isLoadingMoreReviews) return;
...
if (reviewForm) {
    reviewForm.addEventListener('submit', async (e) => {
        ...
        if (text.length > 400) {
            alert('Отзыв слишком длинный. Максимум 400 символов.');
            return;
        }
