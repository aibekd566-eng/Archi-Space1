...
if (heroVideo) {
    ...
    heroVideo.addEventListener('loadeddata', () => {
        ...
        // Show hint animation on audio button after video loads
        if (audioToggle) {
            setTimeout(() => {
                audioToggle.classList.add('hint');
                setTimeout(() => {
                    audioToggle.classList.remove('hint');
                }, 6000);
            }, 2000);
        }
    });

    // Handle video play errors (mobile browsers may block autoplay)
    ...
    heroVideo.play().catch((error) => {
        ...
        // Video will play muted, which is already set
    });
}
...
if (audioToggle && heroVideo) {
    ...
    audioToggle.addEventListener('click', () => {
        if (isMuted) {
            ...
            heroVideo.play().catch((error) => {
                console.log('Cannot play with sound:', error);
                // Revert to muted if failed
...
// Mobile optimization - reduce video quality on mobile for better performance
if (window.innerWidth <= 768 && heroVideo) {
    // Video will still play but with lower bandwidth usage
    heroVideo.setAttribute('preload', 'auto');
}

// Ensure video plays on mobile after user interaction
document.addEventListener('touchstart', function initVideoOnMobile() {
    if (heroVideo && heroVideo.paused) {
        heroVideo.play().catch(() => { });
    }
    // Remove listener after first touch
    document.removeEventListener('touchstart', initVideoOnMobile);
